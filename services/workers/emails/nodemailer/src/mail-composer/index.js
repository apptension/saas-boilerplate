...
class MailComposer {
      ...
      .map((attachment, i) => {
        ...
        data = {
          ...
          contentDisposition:
            ...
            (isMessageNode ? "inline" : "attachment"),
  ...
  _createContentNode(parentNode, element) {
    ...
    if (!/^text\//i.test(element.contentType) || element.contentDisposition) {
      node.setHeader(
        "Content-Disposition",
        element.contentDisposition || (element.cid ? "inline" : "attachment")
      );
    }
  ...
  _processDataUrl(element) {
    let parts = (element.path || element.href).match(
      /^data:((?:[^;]*;)*(?:[^,]*)),(.*)$/i
    );
    if (!parts) {
      return element;
    ...
    element.content = /\bbase64$/i.test(parts[1])
      ...
      : Buffer.from(decodeURIComponent(parts[2]));
