...
class MimeNode {
      ...
      if (
        ...
        if (/^text\//i.test(contentType)) {
          ...
          if (this._isPlainText && !this._hasLongLines) {
            ...
            transferEncoding =
              this.transferEncoding === "B" ? "base64" : "quoted-printable";
          }
      ...
      if (this.content) {
        ...
        let createStream = () => {
          if (["quoted-printable", "base64"].includes(transferEncoding)) {
            contentStream = new (transferEncoding === "base64"
              ? base64
              : qp
            ).Encoder(options);
  ...
  _getTextEncoding(value) {
    ...
    if (!encoding) {
      ...
      ).length; // eslint-disable-line no-control-regex
  ...
  _generateMessageId() {
    return (
      ...
      (this.getEnvelope().from || this.hostname || os.hostname() || "localhost")
        .split("@")
        .pop() +
      ">"
    );
