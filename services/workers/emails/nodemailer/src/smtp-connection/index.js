...
class SMTPConnection extends EventEmitter {
  ...
  _setEnvelope(envelope, callback) {
    ...
    if (!this._envelope.to.length) {
      return callback(
        this._formatError("No recipients defined", "EENVELOPE", false, "API")
      );
    }
  ...
  _actionAUTH_LOGIN_USER(str, callback) {
    ...
    this._sendCommand(
      Buffer.from(this._auth.credentials.user + "", "utf-8").toString("base64")
    );
  }
  ...
  _actionAUTH_LOGIN_PASS(str, callback) {
    ...
    this._sendCommand(
      Buffer.from(this._auth.credentials.pass + "", "utf-8").toString("base64")
    );
  }
