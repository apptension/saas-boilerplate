...
BASE_REPO_URI=$AWS_ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/$PROJECT_NAME-workers-base
...
docker pull "$BASE_REPO_URI:latest" || true

docker build -f base.Dockerfile -t "$BASE_REPO_URI:$VERSION" .
docker tag "$BASE_REPO_URI:$VERSION" "$BASE_REPO_URI:latest"

docker push "$BASE_REPO_URI:$VERSION"
