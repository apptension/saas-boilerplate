...
def get_role_names(user: User) -> List[str]:
    return [group.name for group in user.groups.all()]  # extra database hit: groups


def get_user_avatar_url(user: User) -> str:
    field = serializers.FileField(default="")
    return field.to_representation(user.profile.avatar.thumbnail)  # extra database hits: profile, avatar
