...
uv run python manage.py init_subscriptions || echo "init_subscriptions skipped (may require Stripe configuration)"

echo "Initializing customer plans..."
uv run python manage.py init_customers_plans || echo "init_customers_plans skipped (may require Stripe configuration)"

# Initialize locales (creates default language configurations)
echo "Initializing locales..."
uv run python manage.py init_locales || echo "init_locales skipped"
...
if [ -f "$TRANSLATIONS_MASTER_FILE" ]; then
    ...
    uv run python manage.py sync_translations "$TRANSLATIONS_MASTER_FILE" || echo "sync_translations skipped"
else
...
if [ -f "$TRANSLATIONS_EXPORT_FILE" ]; then
    ...
    uv run python manage.py import_translations "$TRANSLATIONS_EXPORT_FILE" || echo "import_translations skipped"
else
