...
class OIDCLoginView(View):
    ...
    def get(self, request, connection_id):
        ...
        cache.set(f"oidc_state_{auth_params['state']}", {
            'connection_id': str(connection_id),
...
def get_tenant_and_check_access(request, tenant_id, require_admin=False):
    ...
    if require_admin and membership.role not in [TenantUserRole.OWNER, TenantUserRole.ADMIN]:
        return None, None, Response(
            {'error': 'Only owners and admins can manage SSO connections'},
            status=status.HTTP_403_FORBIDDEN
        )
...
class SSODiscoverView(APIView):
    ...
    def get(self, request):
        ...
        require_sso = any(
            getattr(conn, 'enforce_sso', False) for conn in unique_connections
        )
...
class AuditLogListView(APIView):
    ...
    def get(self, request, tenant_id):
        ...
        if include_filter_options:
            ...
            event_types = SSOAuditLog.objects.filter(tenant=tenant).values_list(
                'event_type', flat=True
            ).distinct()
            filter_options['eventTypes'] = list(event_types)
            
            # Get unique user emails
            user_emails = SSOAuditLog.objects.filter(
                tenant=tenant, user__isnull=False
            ).values_list('user__email', flat=True).distinct()[:100]
...
class SCIMTokenListView(APIView):
    ...
    def post(self, request, tenant_id):
        ...
        active_connections = TenantSSOConnection.objects.filter(
            tenant=tenant,
            status=SSOConnectionStatus.ACTIVE
        )
