...
class WebAuthnService:
    ...
    def verify_registration(
        ...
        if client_data.get('origin') != self._get_origin():
            logger.warning(
                f"Origin mismatch: expected {self._get_origin()}, got {client_data.get('origin')}"
            )
    ...
    def verify_authentication(
        ...
        try:
            auth_data_bytes = base64.urlsafe_b64decode(
                authenticator_data + '=' * (-len(authenticator_data) % 4)
            )
    ...
    def list_passkeys(self) -> List[UserPasskey]:
        ...
        return list(UserPasskey.objects.filter(
            user=self.user,
            is_active=True,
        ).order_by('-created_at'))
