...
def process_ai_translation_job(self, job_id: int):
    ...
    try:
        ...
        if not job.overwrite_existing:
            existing_translation_keys = Translation.objects.filter(
        ...
        for i in range(0, len(all_keys), batch_size):
                ...
                with transaction.atomic():
                    for idx, result in enumerate(results):
                        ...
                        if result.success and result.translated_text:
                            ...
                            logger.warning(
                                f"Failed to translate key {result.key}: {result.error}"
                            )
            ...
            logger.info(
                f"Job {job_id}: Processed {job.processed_keys}/{job.total_keys} keys "
                f"({job.failed_keys} failed)"
            )
        ...
        logger.info(
            f"Job {job_id} completed: {job.successful_translations} translations, "
            f"{job.failed_keys} failures"
        )
...
@shared_task
def translate_single_key_async(
    key_id: int,
