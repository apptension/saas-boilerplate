...
class TestTranslationSyncer:
    ...
    def test_get_sync_status(self, db, english_locale, polish_locale, translation_keys):
        ...
        TranslationFactory(key=translation_keys[0], locale=english_locale, status=Translation.Status.PUBLISHED)
        TranslationFactory(key=translation_keys[0], locale=polish_locale, status=Translation.Status.PUBLISHED)
