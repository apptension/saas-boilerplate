...
class TestAITranslationService:
    ...
    @patch('apps.translations.ai_service.OpenAI')
    @patch('apps.translations.ai_service.settings')
    def test_translate_batch_success(self, mock_settings, mock_openai):
        ...
        mock_response.choices = [
            Mock(message=Mock(content='{"translations": {"key1": "Witaj", "key2": "Świat"}}'))
        ]
        mock_client = Mock()
    ...
    def test_translate_batch_partial_response(self, mock_settings, mock_openai):
        ...
        mock_response = Mock()
        mock_response.choices = [
            Mock(message=Mock(content='{"translations": {"key1": "Witaj"}}'))
        ]
        mock_client = Mock()
        mock_client.chat.completions.create.return_value = mock_response
        mock_openai.return_value = mock_client
    ...
    def test_translate_single(self, mock_settings, mock_openai):
        ...
        mock_response = Mock()
        mock_response.choices = [
            Mock(message=Mock(content='{"translations": {"test_key": "Przetłumaczony tekst"}}'))
        ]
        mock_client = Mock()
        mock_client.chat.completions.create.return_value = mock_response
        mock_openai.return_value = mock_client
