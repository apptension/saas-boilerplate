...
class TranslationsQuery(graphene.ObjectType):
    ...
    available_locales = graphene.List(LocaleType, description="Get all active locales")
    default_locale = graphene.Field(LocaleType, description="Get the default locale")
    translation_sync_status = graphene.Field(
        TranslationSyncStatusType, description="Get translation sync status (admin only)"
    )
    locale_progress = graphene.List(LocaleProgressType, description="Get translation progress for all locales")
...
class RollbackTranslationsMutation(relay.ClientIDMutation):
    ...
    def mutate_and_get_payload(cls, root, info, version_id):
        ...
        return RollbackTranslationsMutation(
            success=True, message=f"Rolled back {version.locale.name} to version {version.version}"
        )
