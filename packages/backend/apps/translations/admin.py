...
class TranslationKeyAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Key Information', {
            'fields': ('key', 'default_message', 'description')
        }),
        ('Metadata', {
            'fields': ('source_file', 'is_deprecated', 'first_seen_at', 'last_seen_at'),
            'classes': ('collapse',)
        }),
    )
    ...
    def translation_status(self, obj):
        ...
        return format_html(
            ...
            color, published, active_locales, icon, published, active_locales
        )
...
class LocaleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Settings', {
            'fields': ('is_default', 'is_active', 'rtl', 'sort_order')
        }),
    )
    ...
    def publish_translations_view(self, request, locale_id):
        ...
        try:
            ...
            messages.success(
                request, 
                f"Successfully published {locale.name} translations as {version.version}"
            )
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return HttpResponseRedirect(
                    reverse('admin:translations_locale_ai_progress', args=[job.id])
                )
        else:
...
class TranslationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Translation', {
            'fields': ('key', 'locale', 'value', 'status')
        }),
        ('Reference', {
            'fields': ('default_message_display',),
            'classes': ('collapse',)
        }),
        ('Audit', {
            'fields': ('translated_by', 'reviewed_by', 'created_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
...
class TranslationVersionAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        'locale', 'version', 's3_key', 'content_hash',
        'published_at', 'published_by', 'translation_count'
    ]
    ...
    def activate_version_view(self, request, version_id):
        ...
        try:
            ...
            messages.success(
                request, 
                f"Activated version {version.version} for {version.locale.name}"
            )
...
class AITranslationJobAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        'source_locale', 'target_locale', 'status', 'total_keys', 'processed_keys',
        'failed_keys', 'skipped_keys', 'error_message', 'failed_key_ids',
        'celery_task_id', 'created_by', 'created_at', 'started_at', 'completed_at',
    ...
    fieldsets = (
        ...
        ('Progress', {
            'fields': ('status', 'total_keys', 'processed_keys', 'failed_keys', 'skipped_keys')
        }),
        ('Errors', {
            'fields': ('error_message', 'failed_key_ids'),
            'classes': ('collapse',)
        }),
        ('Tracking', {
    ...
    def progress_display(self, obj):
        ...
        return format_html(
            ...
            '{}%</div></div>',
            max(progress, 5) if progress > 0 else 0, color, progress
        )
    ...
    def stats_display(self, obj):
        ...
        return format_html(
            '<span title="Successful: {}, Failed: {}">{}</span>',
            success, failed, ' / '.join(parts)
        )
    ...
    def retry_job_view(self, request, job_id):
        ...
        try:
            ...
            if job.status == AITranslationJob.Status.FAILED:
                ...
                return HttpResponseRedirect(
                    reverse('admin:translations_locale_ai_progress', args=[job_id])
                )
            else:
    ...
    def retry_jobs(self, request, queryset):
        ...
        if count > 0:
            messages.success(
                request, 
                f"Reset {count} job(s). Click the Retry button on each job to start processing."
            )
        else:
