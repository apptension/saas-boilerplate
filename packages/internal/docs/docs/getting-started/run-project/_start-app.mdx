...
Alternatively, you can use the CLI shorthand to start both backend and frontend with a single command:
...
- Automatically start the webapp development server
...
Mailcatcher is running on `http://localhost:1080`. Check the [dedicated doc](/working-with-sb/dev-tools/mailcatcher) before using.
