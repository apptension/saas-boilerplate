...
`pdm --version`)
  - you need this one if you want to run `pdm add` command in `packages/backend` or `packages/workers` outside
docker container
...
(which can be checked by running `aws --version`)
  - you need this one if you want to deploy <ProjectName/> to AWS from your machine
