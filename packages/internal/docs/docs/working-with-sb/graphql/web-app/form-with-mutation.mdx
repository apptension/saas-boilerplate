...
import { ProductFormFields } from './productForm.types';

export const useCreateProductForm = () => {
  const form = useApiForm<ProductFormFields>();

 const handleSubmit = form.handleSubmit(async (data: ProductFormFields) => {
...
// highlight-start
  const [commitMutation] = useMutation(createProductMutation, {
    onError: (error) => {
      ...
      if (graphQLErrors) {
        form.setApolloGraphQLResponseErrors(graphQLErrors);
      }
    }
  });
// highlight-end
...
- ✅ Commit mutations inside custom hooks to decouple logic from presentation
- ✅ Use `react-intl` for all user-facing text (labels, placeholders, errors)
- ✅ Set `async` on your submit handler to enable loading states
...
<details>
<summary><strong>Mutation not triggering</strong></summary>

Ensure your GraphQL mutation name matches the backend schema. Run `pnpm nx run webapp-api-client:graphql:generate-types` to regenerate types after backend changes.

</details>

<details>
<summary><strong>Form errors not displaying</strong></summary>

Make sure you're passing errors to the `error` prop of your Input component:
```tsx
...
<details>
<summary><strong>TypeScript errors on mutation variables</strong></summary>

Regenerate types with `pnpm nx run webapp-api-client:graphql:generate-types` after any GraphQL schema changes.
...
| Guide | Description |
|-------|-------------|
| [Add Backend Mutation](/working-with-sb/graphql/backend/adding-new-mutation) | Create the backend API for your form |
| [Update Schema Types](/working-with-sb/graphql/web-app/update-schema) | Regenerate TypeScript types |
| [Create React Component](/working-with-sb/guides/web-app/create-react-component) | Component structure patterns |
| [Testing](/working-with-sb/tests/webapp) | Write tests for your forms |
