...
**Option 1: Console Backend** - Emails appear in Celery worker logs (`docker compose logs celery_default`)
```bash
EMAIL_BACKEND=django.core.mail.backends.console.EmailBackend
EMAIL_FROM_ADDRESS=noreply@localhost
```

**Option 2: Mailcatcher** - Emails appear in web UI at [`localhost:1080`](http://localhost:1080)
```bash
...
See the [Mailcatcher documentation](/working-with-sb/dev-tools/mailcatcher) for more details.
:::
...
In production, the email handler uses [AWS SES](https://docs.aws.amazon.com/ses/index.html) to send the email. For local development,
emails are sent to [Mailcatcher](/working-with-sb/dev-tools/mailcatcher) when properly configured.
