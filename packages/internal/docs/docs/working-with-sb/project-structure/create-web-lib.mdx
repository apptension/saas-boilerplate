...
// highlight-start
      "@sb/webapp-mylib": [
        "packages/webapp-libs/webapp-mylib/src/index.ts"
      ],
      "@sb/webapp-mylib/*": [
        "packages/webapp-libs/webapp-mylib/src/*"
      ]
// highlight-end
    }
  }
...
:::note
Follow the existing naming convention: packages are named `webapp-{name}` (e.g., `webapp-core`, `webapp-finances`)
and use the `@sb/webapp-{name}` import path.
:::
...
```ts
import * as mylib from '@sb/webapp-mylib';
```