...
<details>
<summary><strong>Test fails with "Unable to find element"</strong></summary>

Use `findBy*` queries instead of `getBy*` for elements that appear after async operations:

```tsx
// ❌ Fails for async content
...
<details>
<summary><strong>Apollo mock not being called</strong></summary>

Ensure your mock query/mutation variables exactly match what the component sends:

```tsx
// If component sends { id: "123" }, mock must match exactly
composeMockedQueryResult(myQuery, {
  variables: { id: "123" },  // Must match exactly!
  data: { ... }
})
```
...
<details>
<summary><strong>Multiple elements with same text</strong></summary>

Use `findAllBy*` or more specific selectors:

```tsx
// ❌ Fails if multiple buttons exist
...
<details>
<summary><strong>Test passes locally but fails in CI</strong></summary>

- Use `waitFor` for timing-sensitive assertions
- Ensure mocks cover all GraphQL queries the component makes
- Check for timezone-dependent date assertions
...
| Guide | Description |
|-------|-------------|
| [Backend Testing](/working-with-sb/tests/backend) | Django/pytest testing patterns |
| [Create React Component](/working-with-sb/guides/web-app/create-react-component) | Includes test file setup |
| [Form with Mutation](/working-with-sb/graphql/web-app/form-with-mutation) | Test forms and mutations |
