...
- set `region` to the AWS region you'd like to use for deployment of SaaS Boilerplate.
...
We created a `make` rule that will automatically use `aws-vault` if it is detected in your system:
