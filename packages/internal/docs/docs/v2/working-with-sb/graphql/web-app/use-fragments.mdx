...
We'll use a query that already exists in SaaS Boilerplate in a very similar form, but with fewer fields to make the
...
SaaS Boilerplate uses [Codegen](https://the-guild.dev/graphql/codegen) to generate typed queries, mutations,
