...
description: Guiding you through the process of creating a new Serverless package
---

In the context of the SaaS Boilerplate, asynchronous jobs are handled by AWS Lambda functions, which are managed by the Serverless Framework.
...
To use the defined AWS Lambda function handler in the SaaS Boilerplate, it needs to be added to the `serverless.yml` file.
