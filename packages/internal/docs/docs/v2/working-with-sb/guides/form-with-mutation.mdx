...
In this guide you'll learn how to use helpers provided by SaaS boilerplate to work with `react-hook-form` to manage
...
[Adding new mutation](./adding-new-mutation) guide.
...
[Create React component](./create-react-component) guide to create an empty one.
...
```ts title="src/routes/productForm/productForm.hooks.ts"
...
[useApiForm](../../api-reference/webapp-api-client/hooks#useapiform) – custom wrapper around
[`useForm`](https://react-hook-form.com/api/useform/) hook from `react-hook-form` that helps you to work with
SaaS Boilerplate GraphQL API.
...
```tsx title="src/routes/productForm/productForm.component.tsx"
...
- [Button](../../api-reference/webapp-core/components#buttonsbutton) – reusable button component from `@sb/webapp-core`.
...
- [Input](../../api-reference/webapp-core/components#formsinput) – Controlled input component compatible with
`react-hook-form`.
...
```ts title="src/routes/productForm/productForm.graphql.ts"
...
## Key points
- Use [useApiForm](../../api-reference/webapp-api-client/hooks#useapiform) hook to control form state
