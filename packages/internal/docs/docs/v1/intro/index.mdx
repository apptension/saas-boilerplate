...
- `packages/webapp/.env` – variables loaded by webpack when you run the local dev server

- `packages/workers/.env` – variables loaded by docker-compose when you invoke worker functions locally

- `packages/backend/.env` – variables loaded by docker-compose when you run the backend server locally

:::tip
This should be actually done automatically as part of the `make setup`
:::

Now, adjust the key values that have to be set in the `packages/backend/.env`:
...
Backend endpoint docs are available on `http://localhost:5001/doc`.

Admin Panel is running on `http://admin.localhost:5001`.

Workers trigger server is running on `http://localhost:3005`.

Mailcatcher is running on `http://localhost:1080`. Check [dedicated doc ](/v1/features/dev-tools/mailcatcher) before using.
