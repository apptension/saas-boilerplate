...
export const Link = (props: LinkProps) => {
  ...
  const renderInternalLink = (
    props: InternalLinkProps | InternalNavLinkProps
  ) =>
    isInternalNavLink(props) ? (
      <RouterNavLinkContainer {...omit(['navLink'], props)}>
        {content}
      </RouterNavLinkContainer>
    ) : (
      <RouterLinkContainer {...(props as InternalLinkProps)}>
        {content}
      </RouterLinkContainer>
    );
  ...
  return (
    <ThemeProvider theme={theme}>
      {isInternalLink(linkProps)
        ? renderInternalLink(linkProps)
        : renderExternalLink(linkProps as ExternalLinkProps)}
    </ThemeProvider>
  );
};
