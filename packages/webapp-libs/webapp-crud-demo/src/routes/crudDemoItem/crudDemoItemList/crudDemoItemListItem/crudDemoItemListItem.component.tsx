...
export const CrudDemoItemListItem = ({ item }: CrudDemoItemListItemProps) => {
  ...
  const intl = useIntl();
  const { hasPermission: canManageCrud } = usePermissionCheck('features.crud.manage');
  ...
  const renderButtonsMenu = () => {
    if (!canManageCrud) return null;
    return <CrudDropdownMenu className="w-40" itemId={data.id} handleDelete={handleDelete} loading={loading} />;
  };
